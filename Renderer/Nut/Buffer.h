...
namespace Nut
{
    ...
    public:
        ...
        std::vector<uint8_t> GetDataFromBuffer(const std::function<void(bool success)>& onCompleted = nullptr,
                                               uint32_t size = 0,
                                               uint32_t offset = 0) const;
    ...
    public:
        ...
        std::shared_ptr<Buffer> BuildPtr(const std::shared_ptr<NutContext>& ctx) const;
    };

    inline std::shared_ptr<Buffer> BufferBuilder::BuildPtr(const std::shared_ptr<NutContext>& ctx) const
    {
        Buffer bff(m_layout, ctx);
        bff.WriteBuffer(m_data);
        return std::make_shared<Buffer>(bff);
    }
    ...
    {
        static_assert(std::is_standard_layout_v<T>, "T must be standard layout to be uploaded to GPU safely.");
        m_layout.size = sizeof(T);

        m_data = const_cast<void*>(static_cast<const void*>(data));
        return *this;
    }
