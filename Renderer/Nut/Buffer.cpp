...
namespace Nut
{
                            ...
                            uint32_t offset) const
    {
        ...
        {
            LogError("Invalid NutContext or WGPUDevice.");
        ...
        {
            LogWarn("MapAsync operation timed out or failed.");
            stagingBuffer.Unmap();
            if (onCompleted) onCompleted(false);
            return false;
        }
                                                   ...
                                                   uint32_t offset) const
    {
        ...
        if (!success)
        {
            LogError("Failed to read buffer data.");
            return {};
        }
        return bufferData;
    }
    ...
    bool Buffer::WriteBuffer(const void* data, uint32_t size, uint32_t ofst)
    {
        ...
        {
            LogError("Invalid NutContext or buffer.");
            return false;
        }
        if (size == 0)
        {
        ...
        else if (size > m_layout.size)
        {
            ...
            if (!m_buffer)
            {
                LogError("Failed to recreate buffer with larger size.");
                return false;
            }
            rebuilt = true;
        }
