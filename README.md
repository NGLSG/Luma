...
* **蓝图节点系统（可编译为 C#）**
* **C++ / C# 热重载互操作**
* **Box2D 物理、Tilemap、UI 自动布局、音频系统**
* **2D A* 寻路系统（NavGrid + Pathfinder）**
* **资源热重载（FileWatcher 自动检测变更）**
* **Skia + Nut 混合渲染管线**
* **自定义材质管线 / 实例化渲染**
...
> **注意**：请确保已设置 `VCPKG_ROOT` 环境变量指向 vcpkg 安装目录。如未安装 vcpkg 依赖，请先执行 `vcpkg install openssl curl ffmpeg`。
...
* **资产系统**：GUID 管理 + 缓存 + FileWatcher 热重载
* **脚本系统**：CoreCLR + P/Invoke 双向调用
* **渲染系统**：Skia + Nut、插值补帧、批处理
* **物理系统**：Box2D 固定步长
* **寻路系统**：2D A* 网格寻路 + NavAgent 自动导航
* **UI 布局系统**：Flexbox 风格自动排列（水平/垂直/换行/对齐）
* **音频系统**：多线程空间混音
* **任务调度**：工作窃取算法
* **动画系统**：参数化状态机
* **粒子系统**：
